...
async def handle_webhook_POST_logic(payload: WebhookPayload) -> dict:
    ...
    """
    # Validar el payload
    first_message = payload.get_first_message()
    if not first_message:
        raise ValueError("No se encontró ningún mensaje en el payload.")
    logger.info(f"Primer mensaje recibido: {first_message}")

    sender_phone_number = payload.get_sender_phone_number()
    if not sender_phone_number:
        raise ValueError("No se encontró un número de teléfono del remitente.")
    logger.info(f"Número de teléfono del remitente: {sender_phone_number}")

    # Responder al mensaje del remitente
    wa_service = WhatsAppService()
    response = await wa_service.send_text_message_to_user(sender_phone_number, "¡Gracias por tu mensaje!")
